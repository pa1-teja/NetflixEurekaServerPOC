package com.example.booglemaps.Repositories;
...
public class MockAddressRepository {
   ...
   public static Address getRandom(){
       ...
       int randomIndexNumber = randomIndexGenerator.nextInt(ADDRESSES.length);

       String address = ADDRESSES[randomIndexNumber];

       String[] partsOfAddress = address.split(",");
       String streetAndNumber = partsOfAddress[0];
       String cityStateAndZip = partsOfAddress[1];

       String[] citySTateAndZipParts = cityStateAndZip.trim().split(" ");

       LinkedList<String> strings = Arrays.stream(citySTateAndZipParts).map(String::trim).collect(Collectors.toCollection(LinkedList::new));

       String zip = strings.pollLast();
       String state = strings.pollLast();
       String city = String.join(" ", strings);
